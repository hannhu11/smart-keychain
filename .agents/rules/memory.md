...
- [x] **Tiêu chuẩn 11 (Khắc Phục Lệch Font Khi Thoát QR & Chữ Đè Lên Nhau):** Đặt lại `spr->setTextDatum(textdatum_t::top_left)` triệt để mỗi frame; tăng khoảng cách dòng lên 15px, vẽ chữ trong suốt (loại bỏ màu nền đen đặc che khuất chân dòng trước); giảm tốc độ đánh máy chậm lại 2/3 ($65\text{ms}$/ký tự).
- [x] **Tiêu chuẩn 12 (Bộ Sưu Tập Phong Cảnh Trái Đất Sống):** Bổ sung 4 bối cảnh Trái Đất tuyệt đẹp trên cả phần cứng và Web Studio: Quỹ Đạo Trái Đất (Earth Orbit 3D), Cực Quang Trái Đất (Aurora Borealis), Núi Phú Sĩ Mùa Xuân (Mt. Fuji Sakura), Đêm Tokyo Mưa (Tokyo Cyber Night).
- [x] **Tiêu chuẩn 13 (Nâng Cấp Thần Kiếm Khớp 100% Ảnh Thật):** Tích hợp Lõi Ngọc Trung Tâm (Central Diamond Core), hoa văn cánh chim chuôi kiếm và quả táo đuôi 3 cánh chuẩn pixel-art theo đúng ảnh `media_1788421671867.png`.
- [x] **Tiêu chuẩn 14 (Kho 150 Animations Độc Quyền Từ 3 Subagents):** Hoàn thành 150 animations chuẩn thiết kế Obsidian Luxury & UI/UX Pro Max chia đều 3 nhóm chuyên sâu: 50 Thánh Khí Thần Thoại (`data/subagent_weapons_relics_50.json`), 50 Cơ Giáp Không Gian Cyber (`data/subagent_scifi_mecha_50.json`), 50 Thú Cưng Pixel 2.5D (`data/subagent_kawaii_anime_50.json`), và danh mục hợp nhất `data/catalog_150_master.json` (tuyệt đối không dùng emoji hay icon rẻ tiền).
